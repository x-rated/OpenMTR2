...
private:
	static void CALLBACK WaitCallback(
		...
		// The threadpool thread that fires this callback is already initialized
		// as a WinRT MTA thread (Windows threadpool = COM MTA by default).
		// Resume the coroutine directly — no PPL task_continuation_context needed.
		context->m_resume();
	}
