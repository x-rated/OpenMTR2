...
{
	// Snapshot options NOW on the calling thread (Qt main thread) before any
	// co_await. Coroutines resume on background threads where Qt widget access
	// is unsafe — they must use these cached values instead of calling options->*.
	cachedPingSize = options->getPingSize();
	cachedInterval = options->getInterval();
	cachedUseDNS   = options->getUseDNS();
	...
	// Detach from the calling thread's COM apartment immediately.
	//
	// winrt::IAsyncAction captures the originating apartment context on
	// construction and uses IContextCallback::ContextCallback (via rpcrt4) to
	// marshal resumptions back into it. The calling thread is the Qt main thread
	// which has no valid WinRT apartment — so the marshal attempt calls
	// RtlActivateActivationContext on a thread-pool thread whose activation-
	// context stack is in an indeterminate state, causing
	// RtlDeactivateActivationContextUnsafeFast to dereference a freed record
	// => 0xC0000005.
	//
	// Switching to the thread pool here before the when_all co_await means all
	// child coroutine resumptions happen on pool threads with no apartment-
	// marshal path back through rpcrt4.
	co_await winrt::resume_background();

	// Capture address by value — not by reference — so the coroutine owns its
	// copy of the address even after DoTrace's stack frame is gone on resumption.
	// Capturing by reference here is UB (upstream fix from WinMTR-refresh v0.99).
	auto threadMaker = [address, this, stop_token](UCHAR i) {
		...
		if (address.si_family == AF_INET) {
			return this->handleICMP(address.Ipv4, stop_token, std::move(current));
		}
		else if (address.si_family == AF_INET6) {
			return this->handleICMP(address.Ipv6, stop_token, std::move(current));
		}
	...
	tracing = false;
	done = true;  // ALL coroutines have fully exited — safe to call DoTrace again
}
...
template<class T>
[[nodiscard("The task should be awaited")]]
winrt::Windows::Foundation::IAsyncAction WinMTRNet::handleICMP(T remote_addr, std::stop_token stop_token, trace_thread mine) {
	using namespace std::literals;
	using traits = icmp_ping_traits<T>;
	T local_addr = remote_addr;
	co_await winrt::resume_background();

	// Initialize a COM Multi-Threaded Apartment on this thread-pool thread.
	//
	// IcmpSendEcho2 (and the underlying IP Helper / NSI APIs) make internal
	// RPC calls to the Network Store Interface Service via winnsi.dll. Those
	// RPC calls internally call RtlActivateActivationContext to set up a COM
	// activation context. On a thread-pool thread that has never been given a
	// COM apartment, the activation context stack is uninitialised, so the
	// matching RtlDeactivateActivationContextUnsafeFast dereferences a freed
	// record => 0xC0000005.
	//
	// CoInitializeEx(MTA) gives this thread a valid COM context so that the
	// RPC/activation machinery inside winnsi.dll has a sane stack to work with.
	// We pair it with CoUninitialize() at the end of the coroutine via RAII.
	struct CoUninitializer {
		const bool owned;
		explicit CoUninitializer(bool o) noexcept : owned(o) {}
		~CoUninitializer() noexcept { if (owned) CoUninitialize(); }
	};
	const HRESULT hrCo = CoInitializeEx(nullptr, COINIT_MULTITHREADED);
	// S_OK = initialized, S_FALSE = already MTA (fine), RPC_E_CHANGED_MODE = already STA (fine too)
	const CoUninitializer coGuard{ SUCCEEDED(hrCo) };

	using namespace std::string_view_literals;
	const auto				nDataLen = this->cachedPingSize;
	...
	while (this->tracing) {
		...
		if (dwReplyCount) {
			...
			const auto intervalInSec = this->cachedInterval * 1s;
...
winrt::fire_and_forget	WinMTRNet::SetAddr(int at, SOCKADDR_INET addr)
{
	...
	co_await winrt::resume_background();
	// Same COM MTA init as handleICMP — GetNameInfoW also triggers RPC/NSI paths
	// that require a valid COM activation context on the calling thread.
	const HRESULT hrCoSetAddr = CoInitializeEx(nullptr, COINIT_MULTITHREADED);
	struct CoUninitializerSetAddr {
		const bool owned;
		explicit CoUninitializerSetAddr(bool o) noexcept : owned(o) {}
		~CoUninitializerSetAddr() noexcept { if (owned) CoUninitialize(); }
	} coGuardSetAddr{ SUCCEEDED(hrCoSetAddr) };
