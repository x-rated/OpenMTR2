...
public:
	...
	[[nodiscard]]
	bool isTracing() const noexcept { return tracing; }
...
private:
	...
	std::atomic_bool	done{ true };  // starts true — no trace running yet

	// Snapshot of options captured on the main thread at DoTrace start.
	// Coroutines read these instead of calling back into IWinMTROptionsProvider
	// (which accesses Qt widgets and is NOT thread-safe from background threads).
	unsigned	cachedPingSize{ 64 };
	double		cachedInterval{ 1.0 };
	bool		cachedUseDNS{ true };
